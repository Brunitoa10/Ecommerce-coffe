          ...
          <CarouselContent className="-ml-2 md:-ml-4">
            ...
            {result !== null && (
              result.map((product: ProductType) => {
                const { id ,slug, images, productName, price, taste, origin} = product;
                return(
                  <CarouselItem key={id} className="md:basis-1/2 lg:basis-1/3 group">
                    <div className="p-1">
                      <Card className="py-2 border border-gray-200 shadow-none">
                        <CardContent className="relative flex items-center justify-center px-6 py-2">
                          <img src={
                            `${process.env.NEXT_PUBLIC_BACKEND_URL}${images[0]?.formats?.small?.url}`
                          } alt="Image featured"/>
                        ...
                        <div className="flex justify-between gap-4 px-8">
                          ...
                          <div className="flex items-center justifi-between gap-3">
                            <p className="px-2 py-1 text-white bg-black rounded-full dark:text-white dark:text-black w-fit">{taste}</p>
